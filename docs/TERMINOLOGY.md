...
Two flavours, distinguished by _what's consumed_:
...
| **Stage**                          | A defined size/silhouette of a crystal. Currently 2: _small_ (~5×5 bbox) and _full_ (~8×8). Each stage has rotations, each rotation has `solidRows` (wall bitmask per row) and `interiorRows` (hollow bitmask per row).                                                                                                                                          | "Small / full crystal"             |
...
**Conflict:** _Lattice_ is also a candidate generic term for "grid maze."
...
Three upgrade _types_ exist, distinguished by their **trigger axis**:
...
| **Contraband** | A _source_ — upgrade picked from a draft after a boss kill (`upgrades/contraband/`). Contraband upgrades have their own data shape (`ContrabandDef`) with refresh-per-fight or cooldown semantics (Gomu Gomu, Get Out of Jail Free) — they are _not_ in the bites type system; the contraband system is structurally distinct. | "Contraband"                                                              |
...
- **Mutation** — one concept, not two. The active generation method +
  mechanic _and_ the draft option that swaps it use the same word; they
  are the same thing.
...
- **Bites upgrade type** — third type alongside passive and consumable,
  distinguished by its _event-gated_ trigger axis. Iron Jaw is the
  canonical resident.
